...
void PatchC0::updateRibsChanged(int udiv, int vdiv)
{
    for(int i=const_ribs;i<segments.size();++i){
        ...
        for(int j=0;j<4;++j){
            PointCAM*p = segments[i]->children()[i];
            man->removePoint(p);
            delete p;
        }
        delete segments[i];
    ...
    for(int i=0;i<patches_u;++i){
        for(int j=0;j<patches_v;++j){
            for(int k=0;k<udiv;++k){
                ...
                /*PointCAM*p0=new PointCAM(glm::vec3{
                                             control[3*i*points_v+3*j]->pos()*(1-u)+
                                             control[(3*i+3)*points_v+3*j]->pos()*u
                                         },man);
                PointCAM*p1=new PointCAM(glm::vec3{
                                             control[3*i*points_v+3*j+1]->pos()*(1-u)+
                                             control[(3*i+3)*points_v+3*j+1]->pos()*u
                                         },man);
                PointCAM*p2=new PointCAM(glm::vec3{
                                             control[3*i*points_v+3*j+2]->pos()*(1-u)+
                                             control[(3*i+3)*points_v+3*j+2]->pos()*u
                                         },man);
                PointCAM*p3=new PointCAM(glm::vec3{
                                             control[3*i*points_v+3*j+3]->pos()*(1-u)+
                                             control[(3*i+3)*points_v+3*j+3]->pos()*u
                                         },man);
                man->addPoint(p0);
                man->addPoint(p1);
                man->addPoint(p2);
            ...
            /*for(int k=0;k<vdiv;++k){
                float u=(k+1.0)/(vdiv+1.0);
                PointCAM*p0=new PointCAM(glm::vec3{
                                             control[(3*i)*points_v+3*j]->pos()*(1-u)+
                                             control[(3*i)*points_v+3*j+3]->pos()*u
                                         },man);
                PointCAM*p1=new PointCAM(glm::vec3{
                                             control[(3*i+1)*points_v+3*j]->pos()*(1-u)+
                                             control[(3*i+1)*points_v+3*j+3]->pos()*u
                                         },man);
                PointCAM*p2=new PointCAM(glm::vec3{
                                             control[(3*i+2)*points_v+3*j]->pos()*(1-u)+
                                             control[(3*i+2)*points_v+3*j+3]->pos()*u
                                         },man);
                PointCAM*p3=new PointCAM(glm::vec3{
                                             control[(3*i+3)*points_v+3*j]->pos()*(1-u)+
                                             control[(3*i+3)*points_v+3*j+3]->pos()*u
                                         },man);
                man->addPoint(p0);
                man->addPoint(p1);
